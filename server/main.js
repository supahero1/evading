...
function create_ws_server() {
  ...
  if(0) {
    app = uWS.SSLApp({
      cert_file_name: "/etc/letsencrypt/live/localhost/fullchain.pem",
      key_file_name: "/etc/letsencrypt/live/localhost/privkey.pem"
    });
  } else {
    app = uWS.App({});
  }
  app.ws("/*", {
    ...
    if(token) {
      function register_self() {
        ...
        req.write(JSON.stringify({ ip: "ws://localhost:8191", players: clients.length, max_players: max_players }));
        req.end();
      }
