...
if(__SECURE_WEBSITE__) {
  ...
  port = 443;
  options = {
    cert: fs.readFileSync("/etc/letsencrypt/live/__SERVER_NAME__/fullchain.pem"),
    key: fs.readFileSync("/etc/letsencrypt/live/__SERVER_NAME__/privkey.pem")
  };
} else {
  http = require("http");
  port = 80;
...
server.on("connection", function(ws) {
  ...
  ws.on("message", function(message, isBinary) {
    ...
    if(message.byteLength > 15) return ws.terminate();
    ws.game_ready = 1;
    message = new Uint8Array(message);
    socket.write(new Uint8Array([message.byteLength + 1, ws.game_id, ...message]));
  });
