...
function oa(a){try{a=atob(a);let c=a.match(/{ (\d+), (\d+) }/g);null==c&&(c=[]);c=c.map(qa=>qa.match(/\d+/g).map(ra=>+ra));if(c.length!=a.match(/}, (\d+)\n};/)[1])return 0;const d=a.match(/{ (\d+), (\d+), 40, \(uint8_t\[\]\){/);if(null==d)return 0;const f=+d[1],w=+d[2];if(1>f||200<f||1>w||200<w)return 0;const O=eval(`[${a.substring(d.index+d[0].length,a.length-5)}]`);if(!(O instanceof Array)||O.length!=f*w)return 0;for(a=0;a<c.length;++a)if(c[a][0]>=f||c[a][1]>=w)return 0;x=f;y=w;t=x;u=y;ja();ka();
B={};for(a=0;a<c.length;++a)B[`${c[a][0]},${c[a][1]}`]=[c[a][0],c[a][1]];A=Object.values(B);v=new Uint8Array(f*w);v.set(O);da();return 1}catch(c){return 0}}
