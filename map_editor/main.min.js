...
function y(){let a=new Uint8Array(w*x);for(let c=0;c<da;++c)if(!(c>=w))for(let d=0;d<t;++d)d>=x||(a[c*x+d]=v[c*t+d]);da=w;t=x;v=a;ea()}let z=0,A=0,B=0,C=.2,D=1;const E=20*x;let F=0,G=E,H=[0,0],I=0,fa=0;var J=0,K=0,L=0,N=0;let O=[0,0],P;var Q=[],R=[];let S=0,T={},U=!0,V=[],ha=0;function ia(){}function ja(){}
function W(){if(S){var a=P/x|0,c=P%x,d=`${a},${c}`;I&&void 0==T[d]?(T[d]=[a,c],X("S",20+40*T[d][0],20+40*T[d][1],0,!1,!1),V[V.length]=T[d]):fa&&void 0!=T[d]&&(delete T[d],ka(),V=Object.values(T))}}function la(){if(window.innerWidth!=z||window.innerHeight!=A||B!=window.devicePixelRatio)U=!0,B=window.devicePixelRatio,z=window.innerWidth,f.width=z*B,A=window.innerHeight,f.height=A*B}la();window.onresize=la;function Y(a,c,d){return a+(c-a)*d}
f.onwheel=function(a){a=.05*-Math.sign(a.deltaY);D=1<D?D+3*a:D+a;D=Math.min(Math.max(D,.2),2)};window.onmousemove=function(a){H=[a.clientX*B,a.clientY*B];{const {g:c,h:d}={g:(H[0]-.5*f.width)/C+F,h:(H[1]-.5*f.height)/C+G},e=Math.floor(c/40);0>e||e>=w?P=-1:(a=Math.floor(d/40),P=0>a||a>=x?-1:e*x+a)}-1!=P&&(I&&!S&&v[P]!=q&&(v[P]=q,ka()),W())};f.onmousedown=function(a){0==a.button?(I=1,-1!=P&&(S||v[P]==q||(v[P]=q,ka()),W())):2==a.button&&(fa=1,W())};
f.onmouseup=function(a){0==a.button?I=0:2==a.button&&(fa=0)};window.onkeydown=function(a){if(!a.repeat)switch(a.keyCode){case 81:S=1;-1!=P&&W();break;case 69:U=ha=1;break;case 38:case 87:L=1;break;case 40:case 83:N=1;break;case 37:case 65:J=1;break;case 39:case 68:K=1}};window.onkeyup=function(a){if(!a.repeat)switch(a.keyCode){case 81:S=0;break;case 38:case 87:L=0;break;case 40:case 83:N=0;break;case 37:case 65:J=0;break;case 39:case 68:K=0}};
window.onbeforeunload=function(a){a.preventDefault();return a.returnValue="Are you sure you want to quit?"};f.oncontextmenu=function(a){a.preventDefault();return!1};function ma(){var a=0!=V.length?" ":"";a=`#include "../consts.h"\n
\n
