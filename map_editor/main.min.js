const aa=document.getElementById.bind(document),b=document.createElement.bind(document),e=aa("canvas"),g=e.getContext("2d"),h=b("canvas"),k=h.getContext("2d"),l=b("canvas"),m=l.getContext("2d"),n=aa("tiles");n.a=n.appendChild.bind(n);const p=["#dddddd","#aaaaaa","#333333","#fedf78"],ba=["Path","Safezone","Wall","Teleport"];let q=0,r=[];function ca(){for(let a=0;a<r.length;++a)r[a].style["text-decoration"]=a==q?"underline":"none"}let da=0,t=0,u=new Uint8Array(0),w=9,x=9;
function y(){let a=new Uint8Array(w*x);for(var c=0;c<da;++c)if(!(c>=w))for(let d=0;d<t;++d)d>=x||(a[c*x+d]=u[c*t+d]);for(c=0;c<z.length;++c)if(z[c][0]>=w||z[c][1]>=x)delete A[`${z[c][0]},${z[c][1]}`],z.splice(c--,1);da=w;t=x;u=a;ea()}let B=0,C=0,D=0,E=.2,F=1;const G=20*x;let H=0,I=G,J=[0,0],K=0,fa=0;var L=0,N=0,O=0,P=0;let Q=[0,0],R;var S=[],T=[];let U=0,A={},V=!0,z=[],ha=0;function ia(){}function ja(){}
function W(){if(U){var a=R/x|0,c=R%x,d=`${a},${c}`;K&&void 0==A[d]?(A[d]=[a,c],X("S",20+40*A[d][0],20+40*A[d][1],0,!1,!1),z[z.length]=A[d]):fa&&void 0!=A[d]&&(delete A[d],ka(),z=Object.values(A))}}function la(){if(window.innerWidth!=B||window.innerHeight!=C||D!=window.devicePixelRatio)V=!0,D=window.devicePixelRatio,B=window.innerWidth,e.width=B*D,C=window.innerHeight,e.height=C*D}la();window.onresize=la;function Y(a,c,d){return a+(c-a)*d}
e.onwheel=function(a){a=.05*-Math.sign(a.deltaY);F=1<F?F+3*a:F+a;F=Math.min(Math.max(F,.2),2)};window.onmousemove=function(a){J=[a.clientX*D,a.clientY*D];{const {g:c,h:d}={g:(J[0]-.5*e.width)/E+H,h:(J[1]-.5*e.height)/E+I},f=Math.floor(c/40);0>f||f>=w?R=-1:(a=Math.floor(d/40),R=0>a||a>=x?-1:f*x+a)}-1!=R&&(K&&!U&&u[R]!=q&&(u[R]=q,ka()),W())};e.onmousedown=function(a){0==a.button?(K=1,-1!=R&&(U||u[R]==q||(u[R]=q,ka()),W())):2==a.button&&(fa=1,W())};
...
window.onbeforeunload=function(a){a.preventDefault();return a.returnValue="Are you sure you want to quit?"};e.oncontextmenu=function(a){a.preventDefault();return!1};function ma(){var a=0!=z.length?" ":"";a=`#include "../consts.h"\n
\n
...
function na(a){try{a=atob(a);let c=a.match(/{ (\d+), (\d+) }/g);if(null==c)return 0;c=c.map(pa=>pa.match(/\d+/g).map(qa=>+qa));if(c.length!=a.match(/}, (\d+)\n};/)[1])return 0;const d=a.match(/{ (\d+), (\d+), 40, \(uint8_t\[\]\){/);if(null==d)return 0;const f=+d[1],v=+d[2];if(1>f||200<f||1>v||200<v)return 0;const M=eval(`[${a.substring(d.index+d[0].length,a.length-5)}]`);if(!(M instanceof Array)||M.length!=f*v)return 0;for(a=0;a<c.length;++a)if(c[a][0]>=f||c[a][1]>=v)return 0;w=f;x=v;ia();ja();A=
{};for(a=0;a<c.length;++a)A[c[a][0]*x+c[a][1]]=[c[a][0],c[a][1]];z=Object.values(A);u=new Uint8Array(f*v);u.set(M);ea();return 1}catch(c){return 0}}
function ea(){S=Array(p.length);T=Array(p.length);var a=0;for(let c=0;c<w;++c)for(let d=0;d<x;++d)null==S[u[a]]&&(S[u[a]]=new Path2D,T[u[a]]=new Path2D),S[u[a]].rect(2*(1.5+40*c),2*(1.5+40*d),74,74),T[u[a]].rect(80*c,80*d,80,80),++a;h.width=80*w;h.height=80*x;l.width=80*w;l.height=80*x;for(a=0;256>a;++a)S[a]&&(k.fillStyle=p[a]+"b0",k.fill(T[a]),k.fillStyle=p[a],k.fill(S[a]),m.fillStyle=p[a],m.fill(T[a]));V=1}
function ka(){var a=R;const c=a%x;a=(a-c)/x;k.clearRect(80*a,80*c,80,80);k.fillStyle=p[q]+"b0";k.fillRect(80*a,80*c,80,80);k.fillStyle=p[q];k.fillRect(2*(1.5+40*a),2*(1.5+40*c),74,74);m.fillStyle=p[q];m.fillRect(80*a,80*c,80,80);V=1}y();let Z=b("button");Z.innerHTML="Export & copy";Z.timeout=-1;
